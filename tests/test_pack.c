...
main ()
{
  const char *station_id = "XFDSN:XX_TEST__L_H_Z";
  int record_length      = 4096;
  uint8_t encoding       = DE_INT32;
  int byteorder          = 1;
  uint8_t pubversion     = 1;
  const char *starttime  = "2012-01-01T00:00:00";
  double sampling_rate   = 1.0f;
  //int num_samples = strlen(textdata);
  int num_samples     = 500;
  const char *outfile = "foo.ms";

  int rv = pack_miniseed (
      (void *)sindata, station_id,
